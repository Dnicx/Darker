...
public class Ground {
	 ...
	 * 
	 * @param file
	 *            : text file contain ground with format "n width height" follow
	 *            by n number of blocks
	 */
	public Ground(Scanner boxs) {
		...
		try {
			...
			for (int i = 0; i < n && boxs.hasNextInt(); i++) {
				...
				// System.out.println(boxs.nextInt() + " " + boxs.nextInt() + "
				// " + boxs.nextInt() + " " + boxs.nextInt() + " ");
			}
		} catch (Exception e) {
	...
	/*
	 * (slow) fill block array with 1 wherever there is collide box
	 */
	...
	/**
	 * (slow) find the top box that character can stand on
	 * 
	 * @param x
	 *            : character's top x position
	 * @param y
	 *            : character's top y position
	 * @param width
	 *            : width of character
	 * @param height
	 *            : height of character
	 ...
	 */
	/*
	 * public int getGround(int x, int y, int width, int height) { int ground =
	 * Integer.MAX_VALUE; for (CollideBox b : box) { if (b.getLeft() < x+width
	 * && b.getRight() > x && b.getTop() > y+height) { if (ground > b.getTop())
	 * { ground = b.getTop(); } } } return ground; }
	 */
