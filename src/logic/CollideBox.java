...
public class CollideBox {
	 ...
	 * @param c1
	 *            : other CollideBox
	 * @return true if this box collide with the other box
	 */
	public boolean isCollide(CollideBox c1) {
		// check if any part of c1 is in THIS
		if (this.startX <= c1.startX && c1.startX <= this.endX) {
			if (this.startY <= c1.startY && c1.startY <= this.endY)
				return true;
			if (this.startY <= c1.endY && c1.endY <= this.endY)
				return true;
		...
		if (c1.startX <= this.startX && this.startX <= c1.endX) {
			if (c1.startY <= this.startY && this.startY <= c1.endY)
				return true;
			if (c1.startY <= this.endY && this.endY <= c1.endY)
				return true;
